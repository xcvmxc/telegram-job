...
Pipeline (subcommands are called in order from ~/.claude/commands/jobs.md):
    ...
    emit-files [--since ISO]            write matches+<stamp>.md to the user's
...
Everything user-specific lives in the folder from config.json:
    ...
    <folder>/matches+<stamp>.md    output
...
def cmd_pull(args: argparse.Namespace) -> int:
    ...
    if not sources:
        summary["errors"].append({
            "error": f"No sources listed in {config.sources_file()}. "
                     "Add at least one channel and run /jobs again.",
        })
    ...
    for ref in sources:
        ...
        try:
            ...
            # `uv` isn't on PATH. Phrase the error so /jobs' redirect heuristic
            # ("not set up") fires instead of leaking a raw traceback.
            summary["errors"].append({
                "channel": ref,
                "error": "uv not found on PATH — the scanner is not set up. "
                         "Install uv or run /jobs-setup.",
            })
            break
...
def _render_table(rows: list[sqlite3.Row], when_iso: str) -> str:
    ...
    lines = [
        "# Matching vacancies",
        "",
        f"Generated by /jobs at {when_iso}. Rows: {len(order)}.",
        "",
        "| Position | Company | Links | Date | Telegram post |",
        "|---|---|---|---|---|",
    ]
    for key in order:
        g = groups[key]
...
def cmd_emit_files(args: argparse.Namespace) -> int:
    """Write matches+<stamp>.md for jobs extracted since --since."""
    ...
    if rows:
        path = out_dir / f"matches+{stamp}.md"
...
def main() -> int:
    ...
    p_emit = sub.add_parser("emit-files", help="Write matches+<stamp>.md.")
